...
const repulsiveForce = (source: Vec2, adj: Vec2, chill: boolean): Vec2 => {
  const forceStrength = chill ? 10 : 100;
  const forceChillout = chill ? 1 / 20 : 1 / 20;
...
const attractiveForce = (source: Vec2, adj: Vec2, chill: boolean): Vec2 => {
  // Yes this is a random number generated by my brain
  const spring = chill ? 0.0001 : 0.026324;
...
export const applyForces = (
  ...
  for (const vertex of vertices) {
    ...
    const chill = ignore.size > 0;

    const repulsiveForce = computeRepulsiveForce(
      vertex,
      vertices,
      arrangement,
      chill
    ...
    const attractiveForce = computeAttractiveForce(
      vertex,
      graph,
      arrangement,
      chill
